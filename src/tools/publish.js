...
const publish = {
  async setWhite(isOpen = 0) {
    await set(redis_keys.PUBLISH, isOpen);
  },
  async getWhiteStatus() {
    let status = await get(redis_keys.PUBLISH);
    return status;
  },
  async addPerson(persons, isAppend = false) {
    ...
    if (list && list.length) {
      if (!isAppend) {
        await del(redis_keys.PUBLISH_WHITE_PERSONS);
      }
      await sadd(redis_keys.PUBLISH_WHITE_PERSONS, ...list);
      return list.length;
    }
    return 0;
  },
  async delPerson(persons) {
    const list = persons.split(',');
    if (list && list.length) {
      await srem(redis_keys.PUBLISH_WHITE_PERSONS, ...list);
      return list.length;
    }
    return 0;
  },
  async queryPerson() {
    return await smembers(redis_keys.PUBLISH_WHITE_PERSONS);
  },
  async cleanPerson() {
    await del(redis_keys.PUBLISH_WHITE_PERSONS);
  },
